#! /usr/bin/env python
"""
A more customizable lattice. Run using the command:

    python lattice_custom.py
    ...
    for i in range(lattice_size):
        for j in range(lattice_size):
            for k in range(lattice_size):
                site_color_matrix[i,j,k] = vector(0,0,1)
...
def color_links(xlinks_color, ylinks_color, zlinks_color, lattice_size):
    ...
    for i in range(lattice_size):
        for j in range(lattice_size):
            for k in range(lattice_size):
                xlinks_color[i,j,k] = vector(0,1,0)
                ylinks_color[i,j,k] = vector(0,1,0)
                zlinks_color[i,j,k] = vector(0,1,0)
...
def size_sites(site_radius, lattice_size):
    ...
    for i in range(lattice_size):
        for j in range(lattice_size):
            for k in range(lattice_size):
                site_radius[i,j,k] = 0.1*site_radius[i,j,k]
    ...
    for j in range(lattice_size+1):
        for k in range(lattice_size+1):
            ...
            cylinder(pos=vector(i,j,k),
                    axis=vector(1,0,0),
                    radius=link_radii_x[i-1,j-1,k-1],
                    color=linkx_color[i-1,j-1,k-1],
                    opacity=op_factor)
            cylinder(pos=vector(i,j,k),
                    axis=vector(0,1,0),
                    radius=link_radii_y[i-1,j-1,k-1],
                    color=linky_color[i-1,j-1,k-1],
                    opacity=op_factor)
            cylinder(pos=vector(i,j,k),
                    axis=vector(0,0,1),
                    radius=link_radii_z[i-1,j-1,k-1],
                    color=linkz_color[i-1,j-1,k-1],
                    opacity=op_factor)
